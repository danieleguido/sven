...
class Glue:
  ...
  def process(self, verbose=False):
    ...
    self.response['meta']['user'] = self.request.user.username if hasattr(self.request, 'user') else "anonymous"

    self.response['meta']['language'] = self.request.LANGUAGE_CODE if hasattr(self.request, 'LANGUAGE_CODE') else None
    ...
    if self.method == 'GET' and 'search' in self.data:
      self.search = self.meta('search', self.data['search'])
      
    if self.method == 'GET' and 'filters' in self.data:
      try:
        self.filters = self.meta('filters', json.loads(self.data['filters']))
      except Exception, e:
    ...
    # order by
    if self.method == 'GET' and 'order_by' in self.data:
      try:
        self.order_by = self.response['meta']['order_by'] = json.loads(self.data['order_by']) # json array
      except Exception, e:
        self.warning( 'order_by', "Exception: %s" % e )

    # limit / offset
    if self.method=='GET' and ( 'offset' in self.data or 'limit' in self.data ) :
      
      if '%s' % self.data.get('limit', 0) == '-1':
        self.offset = 0
        self.limit = -1
        self.response['meta']['offset'] = self.offset
        self.response['meta']['limit'] = self.limit
      else:
        form = OffsetLimitForm( self.data )
        if form.is_valid():
  ...
  def json( self, mimetype="application/json" ):
    if self.request is not None and self.data.has_key('indent'):
      return HttpResponse(json.dumps(self.response, default=Glue.encoder, indent=4),  content_type=mimetype)
    return HttpResponse(json.dumps(self.response, default=Glue.encoder), content_type=mimetype)
