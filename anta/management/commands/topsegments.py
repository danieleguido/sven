...
class Command(BaseCommand):
  ...
  def handle(self, *args, **options):
    ...
    print [ [s.stemmed, s.max_tfidf, s.max_tf, s.aliases] for s in Segment.objects.raw( 
      """
      SELECT s.id, s.stemmed, s.content, max(ds.tfidf) as max_tfidf, max(ds.tf) as max_tf, count(distinct d.id) as aliases
